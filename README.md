...
| 负荷曲线 | **合成数据** —— 上游数据集只有光伏/气象列，没有负荷。用内置典型日双峰曲线 |
| 电价 | 二次成本 `0.5·Pg² + 20·Pg`（非 case33mg 原始的线性 20 $/MW） |

> 负荷曲线是合成的，**拿它做定量结论前必须先替换成真实负荷数据**。
> `meta.load_source` 里会标明。
...
**结果（Vmax 惩罚 γ=50000，150 个样本外场景）**：

| 方案 | E[成本] $/d | E[切负荷] MWh | CVaR95 | 最坏 | 越限率 |
|---|---|---|---|---|---|
| DET | 1741.9 | 0.857 | 1800.3 | 1815.0 | 0.0% |
| SAA | 1742.4 | 0.853 | 1802.4 | 1817.1 | 0.0% |
| **DRO** | **1742.4** | **0.853** | **1802.4** | **1817.1** | **0.0%** |
