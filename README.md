...
以 **IEEE 33 节点配电网**为载体的微电网优化研究平台：把光伏**概率预测**接进
日前调度，再用 **MATPOWER 全交流潮流**对每一个优化解做物理校验。

> **设计原则：简化模型负责优化，全交流潮流负责说真话。**
> 线性化/直流模型让问题可解，但它们看不见电压、忽略线损 —— 所以每一层的解
> 都要回灌 MATPOWER 校验，并把简化模型与真值的 gap 量出来。
...
> **一条 3.7 MW 的配电馈线，究竟能接多少光伏？瓶颈在哪？**

| 发生了什么 | 渗透率 |
|---|---|
| 最低电压达标 0.95（治好基态就有的欠压） | **56.5%** |
| 网损最低（89.6 kW，比基态降 58%） | 80% |
| 变电站由"送电"转为"受电"（开始反送） | 102.7% |
| **电压越上限 1.05** | **171.8%** ← 真正的瓶颈 |
| 支路过载（热容量） | 237.7% |
| （对比）电压上限若取宽松的 1.10 | 244.9% |

**三条结论：**

1. **瓶颈是电压，不是热容量。** 支路负载率全程最高才 108%，而电压在 172% 就顶到上限。
2. **电压限值怎么取，直接改变结论。** 1.05 与 1.10 让承载力差 **43%** —— 用宽松值会严重高估。
3. **弃光的主因是"禁止反送"，不是电压。** 交流最优潮流里光伏饱和在 3.814 MW
...
| 试了什么 | 结果 | 为什么 |
|---|---|---|
| 分布鲁棒优化（DRO） | 与随机规划差异 < 0.15% | 储能杠杆太小（3 MWh vs 89 MWh/天），不确定性仅 ±12% |
| 放开变电站电压调压 | 优化器完全不用这个自由度 | LinDistFlow **没有线损项**，抬高电压不省任何损耗 |
| 多时段 SOCP 优化 | 求解器不收敛 | `coneprog` 在"优化模式"下失效，解违反平衡 4.16 MW |

### 一个可操作的工程规律

储能**选址**与**定容**耦合，但作用不同（96 组合实测）：

- **选址决定"能不能解决问题"** —— 放错位置时，加大容量毫无用处
  （bus 18：0.5→2.0 MW，越限反而从 8 涨到 9）
- **定容决定"够不够"** —— 放对位置后存在阈值
  （bus 4：0.5 MW 不够，1.0 MW 刚好，再加到 2.0 MW 一分钱不省）

**先选位置，再谈容量。**

---

> 📄 **上面这些结论的可信度是有边界的** —— 数据来源、外部验证、以及否定结果的适用条件，
> 都写在 **[`docs/客观评估.md`](docs/客观评估.md)** 里。
> 一句话预告：**方法层面完整并经过验证，但数据是拼凑的，且有几条技术路径尚未走通。**
>
> 📄 面向不熟悉电力系统的读者写的完整说明见 [`docs/项目说明.md`](docs/项目说明.md)
> （含大白话概念解释、MATPOWER 在本项目中的具体角色、术语表）。
...
│   ├── make_load_profile.py         生成日负荷曲线（IEEE RTS-79 / 可换数据源）
│   └── analyze_cloud_correlation.py 从卫星云图序列提取场景时间相关性
├── data/           导出的数据
│   ├── pv_quantile.csv              4400 行全时段 11 分位数（kW）
│   ├── pv_daily_profile*.csv        96 点 / 24 点标幺日内曲线
│   ├── load_profile.csv             当前生效的日负荷形状（换成自己的即自动切换）
│   └── cloud_correlation.json       云图实测的时间相关系数
├── cases/
│   └── case33mg_der.m               微电网算例：PV + 储能 + 热限值 + 爬坡率
├── src/            全部 MATLAB 脚本（按阶段）
│   ├── demo_matpower.m              MATPOWER 快速体验
│   ├── test_p1_regression.m         P1 回归闸门（25 项）
│   ├── sweep_pv_penetration.m       P2 渗透率扫描（数据落盘）
│   ├── reconfigure.m                P2 网络重构 + 环网穷举
│   ├── plot_network.m               P1 拓扑与潮流方向可视化
│   ├── run_ed.m                     P3 单时段 AC 经济调度
│   ├── build_most_data.m            P4 装配 MOST 输入
│   ├── run_most_24h.m               P4 24h 调度 + AC 校验
│   ├── distflow_lin.m / distflow_eval.m        P5 LinDistFlow 线性化模型
│   ├── opt_dispatch_lindistflow.m   P5 单/多时段调度线性规划
│   ├── opt_dispatch_socp.m          P5 二阶锥 SOCP（潮流模式已验证精确）
│   ├── opt_dispatch_socp_mp.m       多时段 SOCP（⚠️ 求解器不收敛，见文档）
│   ├── make_day_data.m              P5 日数据（复用 P4 曲线，保证一致）
│   ├── solve_multi_scenario.m       多场景 LP（DET / SAA / DRO 共用）
│   ├── gen_pv_scenarios.m           场景生成（时空相关，默认参数来自云图）
│   ├── eval_oss.m                   样本外评估
│   ├── dro_24h.m                    P5 三方对比主实验
│   ├── test_p5_distflow.m           LinDistFlow 精度对标
│   ├── compare_distflow_models.m    三模型精度对比（同注入）
│   ├── compare_mp_models.m          多时段模型对比
│   ├── validate_dispatch_ac.m       解回灌 AC 校验
│   ├── optimize_storage_siting.m    扩展：储能选址 + 定容二维扫描
│   ├── test_vroot.m                 扩展：变电站电压放开
│   ├── plot_overview.m              主线八面板总览
│   ├── plot_extensions.m            扩展九面板总览
│   ├── plot_load_profile.m          负荷曲线对比
│   └── plot_siting_2d.m             选址定容二维可视化
├── results/
│   ├── data/       19 个 CSV（见 results/README.md 索引）
│   └── figures/    20 张图
└── docs/
    └── 项目说明.md                  完整说明（含大白话概念解释、MATPOWER 角色、术语表）
```
...
### 储能选址 + 定容 `src/optimize_storage_siting.m`

二维扫描：**32 个母线 × 3 档容量 = 96 个组合**，每个组合都求解调度并回灌交流校验。
容量按 3 小时时长配套能量。

![储能选址定容二维扫描](results/figures/storage_siting_2d.png)

| | cap 0.5 MW | cap 1.0 MW | cap 2.0 MW |
|---|---|---|---|
| **bus 4**（最优附近） | 2 越限 | **0 越限** | **0 越限**（成本不再降） |
| **bus 7** | 0 越限 | 0 越限 | 0 越限 |
| **bus 18**（最弱末端） | 8 越限 | 9 越限 | **9 越限** ← 加大容量没用 |

**两条规律（图 (c) 是核心）**：

1. **在错的母线上，加大容量毫无用处** —— bus 18 从 0.5 加到 2.0 MW，
   越限反而从 8 涨到 9。因为问题出在"电压被本地注入压垮"，而不是"储能不够"。
2. **在对的母线上，容量存在阈值** —— bus 4 的 0.5 MW 不够（2 个时段越限），
   1.0 MW 就够（0 个），**再加到 2.0 MW 成本一分不降（都是 1265.7 $/d）**。

**工程含义**：**选址决定"能不能解决问题"，定容决定"够不够"。**
先选对位置，再谈容量 —— 反过来做会把钱花在无效的地方。

> 图 (b) 还揭示一件事：**成本在各母线之间几乎均匀（只差 0.4%）**，
> 光看经济性根本选不出位置；真正拉开差距的是图 (a) 的电压越限时段数。
> 所以选址的目标函数不能只写成本。
...
#### ⚠️ 多时段 SOCP：已实现但**求解器不收敛**（未验证）

把同一套 SOCP 方程推广到 24 小时（加储能跨时段耦合，4129 变量、768 个二阶锥，
见 `src/opt_dispatch_socp_mp.m`）后：

- **能求出解，7 秒左右**（`coneprog` 在这个规模上是跑得动的）
- **但 `exitflag = -7`，且解严重违反功率平衡** —— 实测最大残差 **4.16 MW**。
  表现为"晚高峰时段自报进口量 0，而负荷明明有 4.48 MW、储能只有 1 MW"，物理上不成立。

所以 `opt_dispatch_socp_mp.m` **内置了平衡残差校验**，并返回
`o.converged` 标志；`src/compare_mp_models.m` 在检测到未收敛时**直接中止**，
不输出误导性的对比数字。

**这是求解器层面的问题，不是模型问题** —— 同一套方程在单时段"潮流模式"下
已被交流潮流验证为**电压精确**（`src/compare_distflow_models.m`，
六个工况误差全为 0.00000）。

进一步的定位（很有信息量）：**把单时段的"优化模式"单独拿来跑，同样报 −7。**
问题不在多时段耦合，而在**优化模式本身**：

| 模式 | 注入 | 结果 |
|---|---|---|
| 潮流模式 | 用 `lb = ub` **钉死** | ✅ 电压与交流潮流精确一致 |
| 优化模式 | P、Q **自由且可为负**（允许反送） | ❌ exitflag −7，解违反平衡 |

试过给 `sm/sx` 加上界（原本无穷大、可行域极大），**无效**。

**要修的话**：变量缩放（电压 ~1、功率 ~1e-3~4、电流平方更小），
或换 MOSEK/Gurobi 的 SOCP 接口。**当前结论：SOCP 可用作精确的潮流/校验工具，
但还不能用于优化。**
...
| 阶段 | 内容 | 关键验证 |
|---|---|---|
| P0 | 数据桥：光伏分位数预测 → 标幺曲线 | 重跑结果与上游**逐比特一致**（最大差 0） |
| P1 | 微电网算例 `case33mg_der.m`（PV + 储能 + 热限值） | 25 项回归全过；关掉 DER 后与 `case33mg` **逐字节一致** |
| P2 | 渗透率扫描 + 网络重构 | **独立复现 IEEE 33 节点经典最优 139.55 kW** |
| P3 | 单时段 AC 经济调度 | MIPS 与 FMINCON 交叉校验（差 < 0.006 MW） |
| P4 | MOST 24h 时序 + AC 校验 | AC 校验揪出 4 个时段欠压（MOST 看不见） |
| P5 | LinDistFlow + Wasserstein DRO + 样本外评估 | 机制验证通过；得到**否定结果** |
| E1 | 储能选址 + 定容二维扫描（96 组合） | 得出"先选址、后定容"的工程规律 |
| E2 | 二阶锥 (SOCP) 潮流模型 | 与交流潮流**电压误差 0.00000**（LinDistFlow 为 0.00252） |
| E3 | 卫星云图 → 场景时间相关性 | ρ_time = 0.970 由云图实测；揭示尾部风险被低估 4% |

### 已尝试但未走通（诚实记录）

| 项 | 状态 | 卡在哪 |
|---|---|---|
| 多时段 SOCP **优化** | ❌ 求解器不收敛 | `coneprog` 在"优化模式"下失效（单时段也失败），解违反平衡 4.16 MW |
| 变电站电压放开（调压降损） | ❌ 模型里价值为 0 | LinDistFlow **没有线损项**，抬高电压不省任何损耗 |
| 储能选址 + 定容**联合优化** | ⚠️ 部分 | 目前是**二维穷举**（96 组合），不是真正的联合优化模型 |

### 待做（按价值排序）

| # | 内容 | 说明 |
|---|---|---|
| 1 | **用本地区实测负荷曲线替换** | 现在用 IEEE RTS-79 标准曲线；换成本地数据结论会更贴合 |
| 2 | **光伏场景用实测空间相关矩阵** | 现在是等相关假设（ρ 各节点相同） |
| 3 | **SOCP 变量缩放** | 有了明确假设（自由变量+可负 → 数值困难），值得一试，但不保证成 |
| 4 | **把 SOCP 用于单时段选址扫描** | 替代 LinDistFlow，让每步电压都精确（但受 #3 阻塞） |
| 5 | **三相不平衡模型** | 实际配电网常需三相 |
| 6 | **N-1 安全性** | MOST 支持事故约束（`contab`），可启用 |
| 7 | **滚动优化 / MPC** | 用不断更新的预测逐步修正决策 |
| 8 | **统一配置文件 + 各阶段回归测试 + CI** | 工程化 |

> 完整的 26 条限制与优化点见 [`docs/项目说明.md`](docs/项目说明.md) 第 10 节。
